# -*- coding: utf-8 -*-
"""ML_HW3_Problem2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/111dnF3nn8DeXTaOpXvcpj_lwULQ4Cr9w
"""
...
def get_L(data, num_clusters, mean_li, cov_li, phi_matrix, pi_vect):
  ...
  for i in range(len(data)):
    for j in range(num_clusters):
      #print("SCIPY:")
      s_res= scipy.stats.multivariate_normal.pdf(data[i], mean_li[j], cov_li[j], allow_singular=True) 
      if s_res !=0:

        #print(len(mean_li) )
        #print(len(cov_li))
        L_func_value += phi_matrix[i][j] *  ( lg(pi_vect[j]) + lg(s_res) )  
...
def plot(L_li):
  ...
  for run in range (number_of_runs):
   # print(len(L_li[run]))
    plt.plot( list(range(5,number_of_EM_iterations+1)) , L_li[run][4:], label=run+1)
  plt.legend()
